...
class XianyuReplyBot:
    ...
    def _init_talk_scripts(self):
        """加载预设话术库"""
        script_path = os.path.join("prompts", "faq_pairs.json")
        if os.path.exists(script_path):
            try:
                with open(script_path, "r", encoding="utf-8") as f:
                    self.faq_pairs = json.load(f)
                logger.info(f"已加载 FAQ 话术 {len(self.faq_pairs)} 条")
            except Exception as e:
                logger.error(f"加载 FAQ 话术失败: {e}")
                self.faq_pairs = []
        else:
            self.faq_pairs = []

        self.talk_script_matcher = TalkScriptMatcher(self.faq_pairs)
    ...
    def generate_reply(self, user_msg: str, item_desc: str, context: List[Dict]) -> str:
        ...
        # 0. FAQ 话术优先匹配
        scripted_reply = self.talk_script_matcher.match(user_msg)
        if scripted_reply:
            logger.info('命中 FAQ 话术，直接回复')
            self.last_intent = 'faq'
            return self._safe_filter(scripted_reply)
...
class TalkScriptMatcher:
    """FAQ 话术匹配器：命中后直接返回固定话术"""

    def __init__(self, faq_pairs: List[Dict]):
        self.faq_pairs = faq_pairs or []
    ...
    def match(self, user_msg: str) -> Optional[str]:
        ...
        query = self._normalize(user_msg)
        if not query:
            return None
        ...
        for item in self.faq_pairs:
            answer = (item.get("answer") or "").strip()
            aliases = item.get("aliases") or []
            if item.get("question"):
                aliases = [item["question"], *aliases]
            ...
            for alias in aliases:
                ...
                if query in alias_norm or alias_norm in query:
                    ...
                    score = difflib.SequenceMatcher(None, query, alias_norm).ratio()
